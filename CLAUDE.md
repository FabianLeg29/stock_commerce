...
- **`saisieStock` / `commerce`** : une collection à écriture continue, un document par produit et par jour (id = `${date}__${slug(produit)}`), interrogées avec un simple filtre `where('date','==', aujourdhui)` (pas d'index composé nécessaire). C'est l'historique complet ; il n'est jamais rechargé en entier par l'app. `commerce` ne porte plus que `ventes` — `achats` et `prixAchat` du jour ont été retirés de ce document et vivent maintenant dans la collection `achats`.
...
- **Commerce** : pour chaque produit actif — dernier stock connu (valeur + date), achats du jour (calculés depuis la collection `achats`, lecture seule ici), ventes du jour (saisie), réservations (quantité promise à un client, pas encore livrée), **stock réel** = dernier stock connu + achats − ventes, et **stock disponible** = stock réel − réservations (recalculés en direct). Un bouton **▸ Détail** par produit déplie une ligne listant les achats du jour (fournisseur, prix, heure, transport) ainsi que les ventes et réservations en cours.
