...
- **`production`** : saisie du stock du soir (onglet Production) ; peut aussi créer un nouveau produit (désignation minimale) depuis cet onglet, mais ne peut pas le modifier ensuite ni le supprimer.
...
- **Production** : produits actifs groupés par **famille** (sous-titres), avec deux champs de saisie **Haut** et **Bas** par produit et un **Total** calculé automatiquement (non modifiable), enregistrement automatique au blocage du champ (`change`). Un mini-formulaire permet aussi d'ajouter un nouveau produit (désignation seule) directement depuis cet onglet. Recherche/filtre en haut. Important : cette saisie n'impacte jamais directement le "dernier stock connu" utilisé par le Commerce — elle passe par la validation (voir onglet Commerce) avant de compter dans le stock réel.
