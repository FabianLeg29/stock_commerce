...
```
Firestore
├── produits/{produitId}     { nom, famille, variete, conditionnement, suremballage, actif, prixAchatDefaut, reservations, ordre }
...
├── commerce/{date__slug}    { date, produit, ventes, horodatage, responsable }
...
Points importants :
- **`produits/{produitId}`** : un document par produit (`produitId` = `slugify(nom)`), pour permettre des règles de sécurité par champ (voir Rôles). Créé/supprimé uniquement par l'admin (onglet Paramètres) ; les champs `actif`/`prixAchatDefaut`/`reservations` peuvent aussi être modifiés par le rôle `commerce`. Chargé en entier au démarrage (`onSnapshot` sur la collection, triée côté client par `ordre`) — volume faible, pas de pagination nécessaire. `reservations` = quantité actuellement promise à un client mais pas encore livrée ; c'est un état qui persiste (contrairement à `achats`/`ventes` qui sont des flux quotidiens) — utilisé pour calculer le **stock disponible** = stock réel − réservations, dans l'onglet Commerce.
...
- **`commerce`** : une collection à écriture continue, un document par produit et par jour (id = `${date}__${slug(produit)}`), interrogée avec `where('date','==', aujourdhui)`. Ne porte plus que `ventes` — `achats` et `prixAchat` du jour ont été retirés de ce document et vivent maintenant dans la collection `achats`.
...
- **`production`** : saisie du stock du soir (onglet Production) ; peut aussi créer un nouveau produit (désignation minimale) depuis cet onglet, et réactiver un produit inactif existant (modifie uniquement le champ `actif`) — ne peut ni le supprimer, ni modifier ses autres champs (famille, variété...).
- **`commerce`** : onglets **Achats** (entrées de marchandise), **Commerce** (ventes/réservations du jour + validation du stock du soir) et **Prix d'achat** (prix par défaut + actif/inactif d'un produit existant) — pas la création/suppression de produit, ni les familles/variétés/conditionnements/suremballages, réservés à l'admin. Cette restriction est appliquée par les règles Firestore elles-mêmes (`produits/{produitId}`, champ par champ), pas seulement par l'interface. C'est la page où chaque commercial (Nadine, Julien, Flo, Mickael, Stéphanie...) saisit ses entrées de marchandise (achats), sorties de marchandise (ventes) et réservations client — catalogue partagé entre tous les commerciaux, pas de filtrage par personne.
...
- **Production** : produits actifs groupés par **famille** (sous-titres), avec deux champs de saisie **Haut** et **Bas** par produit et un **Total** calculé automatiquement (non modifiable), enregistrement automatique au blocage du champ (`change`). Un mini-formulaire permet aussi d'ajouter un nouveau produit (désignation seule) directement depuis cet onglet, et un second bouton ouvre un panneau listant les produits **inactifs** (avec recherche) pour en réactiver un en un clic plutôt que d'en recréer un doublon. Recherche/filtre en haut. Important : cette saisie n'impacte jamais directement le "dernier stock connu" utilisé par le Commerce — elle passe par la validation (voir onglet Commerce) avant de compter dans le stock réel.
...
- **Commerce** : pour chaque produit actif — dernier stock connu (valeur + date), achats du jour (calculés depuis la collection `achats`, lecture seule ici), ventes du jour (saisie), réservations (quantité promise à un client, pas encore livrée), **stock réel** = dernier stock connu + achats − ventes, et **stock disponible** = stock réel − réservations (recalculés en direct). Un bouton **▸ Détail** par produit déplie une ligne listant les achats du jour (fournisseur, prix, heure, transport) ainsi que les ventes et réservations en cours. En dessous, section **Stock du soir à valider** : liste des saisies de production pas encore importées (`saisieStock.importe == false`), avec Haut/Bas/Total, un bouton de validation par ligne et un bouton « Tout valider » — la validation copie la valeur dans `dernierStock` et marque l'entrée comme importée.
- **Prix d'achat** : tous les produits (actifs ou non) avec leur prix d'achat par défaut et leur statut actif/inactif (saisonnalité), modifiables par le rôle `commerce` — les autres colonnes (famille, variété, conditionnement, suremballage) y sont affichées en lecture seule.
